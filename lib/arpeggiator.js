...
function generateChordRhythmEvents(songMidiData, CHORD_LIB_GLOBAL, NOTE_NAMES_GLOBAL, helpers, slotContext) {
    ...
    // Section-type family filtering for arpeggio patterns
    ...
    if (sectionType.includes('verse') || sectionType.includes('intro')) {
        const subset = {};
        versePatternKeys.forEach(k => { if (ARPEGGIO_PATTERNS[k]) subset[k] = ARPEGGIO_PATTERNS[k]; });
        if (Object.keys(subset).length > 0) filteredPatterns = subset;
    } else if (sectionType.includes('chorus')) {
        const subset = {};
        chorusPatternKeys.forEach(k => { if (ARPEGGIO_PATTERNS[k]) subset[k] = ARPEGGIO_PATTERNS[k]; });
        if (Object.keys(subset).length > 0) filteredPatterns = subset;
    } else if (sectionType.includes('bridge')) {
        const subset = {};
        bridgePatternKeys.forEach(k => { if (ARPEGGIO_PATTERNS[k]) subset[k] = ARPEGGIO_PATTERNS[k]; });
        if (Object.keys(subset).length > 0) filteredPatterns = subset;
    }
    ...
    while (currentTickInSlot < durationTicks) {
        ...
        if (durationInBeats > 0) {
            rhythmicEvents.push({
                ...
                velocity: humanizeVelocity(72, 15, (startTickAbsolute + currentTickInSlot) % ticksPerBeat, ticksPerBeat)
            });
        }
